...
function normalizeTag(raw?: string): JournalTag {
  if (!raw) return "ALL";
...
function tagToSlug(tag: JournalTag): string {
  if (tag === "ALL") return "";
  if (tag === "Design Archive") return "design-archive";
...
export default function JournalClient({
  ...
  initialTagParam?: string;
}) {
  const initialTag = useMemo(() => normalizeTag(initialTagParam), [initialTagParam]);
  ...
  const feedItems: UnifiedFeedItem[] = useMemo(() => {
    const p01Items: UnifiedFeedItem[] = PROJECT_01_JOURNAL_ENTRIES.map((entry) => {
      ...
      if (entry.category.toLowerCase().includes("chassis")) tag = "Chassis";
        ...
        entry.category.toLowerCase().includes("race") ||
        entry.category.toLowerCase().includes("racing")
      )
        tag = "Racing";
    ...
    return [...p01Items, ...articleItems, ...archiveItems].sort((a, b) =>
      a.date < b.date ? 1 : -1
    );
  }, []);
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {tags.map((tag) => {
              ...
              const count =
                tag === "ALL"
                  ? feedItems.length
                  : feedItems.filter((i) => i.tag === tag).length;
              const tagSlug = tagToSlug(tag);
              const href = tagSlug ? `/journal?tag=${tagSlug}` : "/journal";
